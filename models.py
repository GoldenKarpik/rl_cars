...
class Vehicle:
    ...
    def __init__(self, x_start, lane, speed, direction):
        ...
        self.safe_distance = (self.speed * 2 + safe_distance) * (
                    2 - self.caution) * 0.5  # безопасная дистанция для преследования
    ...
    def if_can_overtake(self, vehicles, vehicle, time):
        ...
        distance = (closest.x - self.direction * closest.length // 2 - self.direction * safe_distance) - \
                   (
                               self.x + self.direction * self.length // 2 + self.direction * safe_distance) - self.direction * max_speed_c * (
                               time + 1)
        # print(distance)
